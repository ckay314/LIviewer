...
if not justIS:
    ...
    otherData = np.genfromtxt(llamaFolder+'otherCats_1.0.dat', dtype=str)
    ...
    ICEdata   = np.genfromtxt(llamaFolder+'LLAMAICE_1.0.csv', delimiter=',', dtype=str)
    HSSfile = llamaFolder+'HSSsort.dat'
    plotbounds = np.genfromtxt(llamaFolder+'plotBounds_1.0.dat', dtype=str)
...
def checkCMEchoice(CMEchoiceIn):
    # ============================== ID whether ICE case or generic ==============================
    # =====  check if date or CME number =====
    # assume given a date if has - in it
    global myID, myoIDs, myYr, myDate, myDT, ICEcase, CMEchoice
    CMEchoice = CMEchoiceIn
    # Assume won't have more than 99,999 LLAMAICE IDs anytime soon
    # and any form of date string requires a min of 8 (YYMMDD maybe but not including)
    if len(CMEchoice) < 5:
    ...
    # Assume a time string, try the options
    else:
        ICEcase = False
        flagEntry = False
        # Check YYYYMMDD
        if len(CMEchoice) == 8:
            try:
                myDT = datetime.datetime.strptime(CMEchoice, "%Y%m%d" )
            except:
                flagEntry = True
        # Check YYYYxMMxDD where x in [- _ / .]
        elif len(CMEchoice) == 10:
            if CMEchoice[4] in ['-', '_', '/', '.']:
                x = CMEchoice[4]
                myForm = '%Y'+x+'%m'+x+'%d'
                try:
                    myDT = datetime.datetime.strptime(CMEchoice, myForm)
                except:
                    flagEntry = True
        # Check YYYMMDDHHMM
        elif len(CMEchoice) == 12:
            try:
                myDT = datetime.datetime.strptime(CMEchoice, "%Y%m%d%H%M" )
            except:
                flagEntry = True
        # Check YYYYxMMxDDTHH:MM
        elif len(CMEchoice) == 16:
            if CMEchoice[4] in ['-', '_', '/', '.']:
                x = CMEchoice[4]
                myForm = '%Y'+x+'%m'+x+'%dT%H:%M'
                try:
                    myDT = datetime.datetime.strptime(CMEchoice, myForm)
                except:
                    flagEntry = True 
                ...
                if not justIS:    
                    # Check if this is a LLAMAICE case since has right time format
                    myDate = myDT.strftime("%Y-%m-%dT%H:%M" )
                    foundFlag = 0
                    if myDate in ICEdata[:,2]:
                        foundFlag = 2
                    elif myDate in ICEdata[:,3]:
                        foundFlag = 3
                    elif myDate in ICEdata[:,4]:
                        foundFlag = 4
                    # If found it pull the other stuff
                    if foundFlag:
                        myID = np.where(ICEdata[:,foundFlag] == myDate)[0][0]
                        CMEchoice = str(myID + 1)
                        myoIDs = np.where(otherData[:,1] == myDate)[0]
                        print (myDate + ' corresponds to LLAMAICE case '+ CMEchoice)
                        ICEcase = True
        else:
            flagEntry = True
        ...
        if not flagEntry:
            ...
            sys.exit('Error in time format, options are YYYYMMDD, YYYY-MM-DD, YYYYMMDDHHMM, YYYY-MM-DDTHH:MM')
                
        


def makeplot(saveIt = False):
